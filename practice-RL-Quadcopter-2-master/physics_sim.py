...
class PhysicsSim():
    ...
    def next_timestep(self, rotor_speeds):
        '''take in actions and compute relevant measures(angles etc.),
        check up the position of copter if reaching lower or upper bound, if so, change flag of done status,
        add in position to new_positions,
        and check if exceeding runtime, if so, change flag of done status
